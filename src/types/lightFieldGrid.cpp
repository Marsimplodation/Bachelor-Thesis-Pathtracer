...
int getLUTIdx(int u, int v, int s, int t, int idx) {
    return (u * grids[idx].size.x * grids[idx].size.y * grids[idx].size.x) +
           (v * grids[idx].size.x * grids[idx].size.y) +
           (s * grids[idx].size.x) + t;
}
...
void constructChannel(float u, float v, float s, float t, int idx) {
    ...
    Vector3 points[] = {
        {grids[idx].min.x + deltaX * u, grids[idx].min.y + deltaY * v,
         grids[idx].min.z},
        {grids[idx].min.x + deltaX * (u + 1.0f),
         grids[idx].min.y + deltaY * (v + 1.0f), grids[idx].min.z},
        {grids[idx].min.x + deltaX * s, grids[idx].min.y + deltaY * (t + 1.0f),
         grids[idx].max.z},
        {grids[idx].min.x + deltaX * (s + 1.0f),
         grids[idx].min.y + deltaY * (t), grids[idx].max.z},
    };
